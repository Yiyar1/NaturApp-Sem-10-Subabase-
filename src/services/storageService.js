...
const KEYS = {
    ...
    THEME_PREFERENCE: '@naturapp_theme_preference',
...
const StorageService = {
    ...
    // --- Preferencias booleanas (legado, ver THEME_PREFERENCE) ---
    ...
    async setThemePreference(pref) {
        await AsyncStorage.setItem(
            KEYS.THEME_PREFERENCE, pref
        );
        // Mantiene compatibilidad con el flag booleano legado
        await AsyncStorage.setItem(
            KEYS.THEME_DARK, JSON.stringify(pref === 'dark')
        );
    },
    ...
    async getThemePreference() {
        ...
        if (val === 'system' || val === 'light' ||
            val === 'dark') {
            return val;
        }
        // Migración: si solo existe el flag booleano legado,
